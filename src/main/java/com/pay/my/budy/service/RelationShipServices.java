...
public class RelationShipServices {
	 ...
	 * 
	 *                  <p>
	 *                  On traite sa liste d'amis pour afficher leurs noms et info
	 *                  on les ajoute dans un friendDTO qui sera ajouté a une
	 *                  listfriendDTO si plusieurs amis puis renvoyer
	...
	/**
	 * 
	 * @param email,    l'email a ajouter on verifiera qu'elle n'existe pas deja en
	 *                  base si ce n'est pas le cas on recupere l'id et on l'ajoute
...
//	
	...
	public String addConnectionBis(String email, String username) {
		...
		if (email.trim() != "" || email != null) {
			...
			friend = new User();
			friend = userRepository.findByusername(email);
			...
			if (friend != null) {
				...
				if (user.getFriends() == null || user.getFriends().isEmpty()) {
					...
					relationShip.setFriend(friend.getId());
					relationShip.setUser_id(user.getId());

					myFriendRelationShip.setFriend(user.getId());
					myFriendRelationShip.setUser_id(friend.getId());
					...
					for (int i = 0; i < user.getFriends().size(); i++) {
						...
						if (user.getFriends().get(i).getFriend() == friend.getId()) {

							return "/layouts/friend_exist";
				...
				return "/layouts/friend_not_exist";
